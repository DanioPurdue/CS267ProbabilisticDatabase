    ...
    for one_query in PD.queries:
        ...
        for one_conj in one_query:
            ...
            for predicate in one_conj:
                print(predicate)
     pd.merge(PD.tables_df['R'].iloc[:,:-1],PD.tables_df['P'].iloc[:,:-1],on = 'var1')
    """
    # DB = PD.tables_dicts
    # print(DB)
    # print(DB["Q"][("1",)])
...
query_inference = GibbsSampling_2.run_Gibbs(PD,100) #Optional positional keyword: steps = # of steps
